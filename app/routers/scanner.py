...
class DomainRequest(BaseModel):
    domain: str = Field(
        ..., pattern=r"^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$", description="A valid domain name to scan"
    )
...
async def scan_dns(request: DomainRequest):
    ...
    try:
        answer_for_A = await resolver.resolve(
            request.domain, "A", lifetime=3.0
        )  # send dns resolve request if there is no domain in redis
    ...
    await redis.set(  # set in redis before adding to postgres
        cached_key, json.dumps(answer_dict), ex=300
    )

    domain = Domain(
        qname=answer_dict["qname"],
        canonical_name=answer_dict["canonical_name"],
        record_type=answer_dict["record_type"],
        record_class=answer_dict["record_class"],
        expiration=answer_dict["expiration"],
        records=answer_dict["records"],
    )
...
async def check_domain(domain: str):
    ...
    if cached_response is not None:
        ...
        response = {"source": "redis", "ttl": ttl, "cached_response": json.loads(cached_response)}
        return response
    print("not in cache")

    async with Session.begin() as session:
        stmt = select(Domain).where(Domain.qname == f"{domain}.").order_by(Domain.id.desc())
